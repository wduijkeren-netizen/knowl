    ...
    return (
      <div className="min-h-screen bg-[#f8f7ff]">
        ...
        <main className="max-w-2xl mx-auto px-4 py-8 space-y-6 text-center">
          <h1 className="text-xl font-bold text-indigo-900">Invuloefening voltooid</h1>
          <div className="bg-white rounded-3xl border border-indigo-100 shadow-sm p-8 space-y-4">
            ...
            <p className="text-sm text-indigo-400">{score} van de {cards.length} correct ingevoerd</p>
            <div className="flex gap-3 justify-center pt-2">
              <button onClick={restart} className="bg-indigo-600 text-white px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-indigo-700 transition-colors">Opnieuw</button>
              <Link href={`/flashcards/${set.id}`} className="bg-indigo-100 text-indigo-700 px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-indigo-200 transition-colors">Terug</Link>
            </div>
          </div>
        </main>
      </div>
      ...
      <main className="max-w-2xl mx-auto px-4 py-8 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <h1 className="text-xl font-bold text-indigo-900 mt-1">Invuloefening — {set.title}</h1>
          </div>
        ...
        <div className="bg-white rounded-3xl border border-indigo-100 shadow-sm p-8 space-y-6">
          ...
          <div className="space-y-3">
            ...
            {result === 'correct' && (
              <p className="text-sm text-emerald-600 font-medium text-center">Correct!</p>
            )}
            {result === 'bijna' && (
              <div className="text-center">
                <p className="text-sm text-amber-600 font-medium">Bijna goed — het juiste antwoord is:</p>
                <p className="text-sm font-bold text-amber-700 mt-1">{card.back}</p>
              </div>
            )}
            {result === 'fout' && (
              <div className="text-center">
                <p className="text-sm text-red-500 font-medium">Niet correct — het antwoord was:</p>
          ...
          {!result ? (
            ...
            <button onClick={handleNext}
              ...
              {index + 1 >= cards.length ? 'Resultaat bekijken' : 'Volgende →'}
            </button>
          )}
        </div>
      </main>
