    ...
    <header className="bg-white/80 backdrop-blur-sm border-b border-indigo-100 sticky top-0 z-10">
      <div className="max-w-3xl mx-auto px-4 py-3 flex justify-between items-center">
        <div className="flex items-center gap-4">
          <Link href="/home" className="text-lg font-bold text-indigo-700 tracking-tight hover:text-indigo-500 transition-colors">Knowl</Link>
          ...
          <div className="relative">
            ...
            {showLangMenu && (
              <>
                ...
                <div className="absolute right-0 top-full mt-2 bg-white rounded-2xl border border-indigo-100 shadow-xl shadow-indigo-100/50 py-2 z-20 min-w-[180px]">
                  {languages.map(l => (
                    <button
                      key={l.code}
                      ...
                      className={`w-full flex items-center gap-3 px-4 py-2 text-sm hover:bg-indigo-50 transition-colors ${
                        lang === l.code ? 'text-indigo-700 font-semibold' : 'text-gray-600'
                      }`}
                    >
                      <span>{l.flag}</span>
                      <span>{l.label}</span>
