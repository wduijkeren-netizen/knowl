    ...
    <div className="min-h-screen bg-[#f8f7ff]">
      ...
      <main className="max-w-3xl mx-auto px-4 py-8 space-y-6">
        <div className="flex items-center gap-3">
          <Link href={`/flashcards/${set.id}`} className="text-indigo-400 hover:text-indigo-600 text-sm">{fc.back}</Link>
        ...
        <div className="bg-white rounded-2xl border border-indigo-100 shadow-sm p-6 space-y-3">
          <div className="flex justify-between items-center">
            <h2 className="font-semibold text-indigo-900">{cards.length} {fc.cardsCount}</h2>
        ...
        <button onClick={handleSave} disabled={saving}
          ...
          {saving ? fc.saving : fc.saveChanges}
        </button>
      </main>
    </div>
  )
